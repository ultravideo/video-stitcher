...
void calibrateMeshWarp2(vector<Mat> &full_imgs, vector<ImageFeatures> features,
    ...
    int images_count = full_imgs.size();

    Eigen::SparseMatrix<double> A(M * N * 2 * 2 * images_count, M * N * 2 * images_count);
    //Identity matrix
    int row = 0;
    ...
    Eigen::VectorXd b(N * M * 2 * 2 * images_count);
    b.fill(0);
    ...
    for (int idx = 0; idx < images_count; ++idx) {
        for (int y = 0; y < N; ++y) {
            for (int x = 0; x < M; ++x) {
                ...
                int constraint_pos = constraint_num * N * M * images_count * dim;
                int image_pos = idx * N * M * dim;
                int mesh_pos_rel = (y * M + x) * dim;

                Size mesh_size = images[idx].size();
                b(((y * M + x) * 2) + idx * N * M * 2) = static_cast<float>(x) * mesh_size.width / (M - 1);
                b(((y * M + x) * 2 + 1) + idx * N * M * 2) = static_cast<float>(y) * mesh_size.height / (N - 1);
            }
        }
    }
    ...
    for (int idx = 0; idx < images_count; ++idx) {
        for (int y = 0; y < N; ++y) {
            for (int x = 0; x < M; ++x) {
                ...
                int constraint_offset = constraint_num * N * M * images_count * dim;
                int image_offset = idx * N * M * dim;
                int mesh_offset = (y * M + x) * dim;

                int mesh_pos = constraint_offset + image_offset + mesh_offset;

                b(mesh_pos) = static_cast<float>(x) * mesh_size.width / (M - 1);
                b(mesh_pos + 1) = static_cast<float>(y) * mesh_size.height / (N - 1);
    ...
    /* float cell_width = static_cast<float>(mat_size.width / M); */
    /* float cell_height = static_cast<float>(mat_size.height / N); */
    /* for (int y = 0; y < N; ++y) { */
    /*     for (int x = 0; x < M; ++x) { */
    /*         float pre_warp_x = cell_width * x; */
    /*         float pre_warp_y = cell_height * y; */
    /*         for (int ft = 0; ft < features[0].keypoints.size(); ++ft) { */
    /*                 Point ft_point = features[0].keypoints[ft].pt; */
    /*                 int tau = 1; */
    /*                 if (sqrt(pow(ft_point.x - pre_warp_x, 2) + pow(ft_point.y - pre_warp_y, 2)) < GLOBAL_DIST) { */
    /*                     tau = 0; */
    /*                     break; */
    /*                 } */
    /*                 A.insert(row, col) = tau * a; */
    /*                 A.insert(row + 1, col + 1) = tau * a; */
    /*                 b(row) = tau * a * pre_warp_x; */
    /*                 b(row + 1) = tau * a * pre_warp_y; */
    /*                 col += 2; */
    /*                 row += 2; */
    /*         } */
    /*     } */
    ...
    for (int idx = 0; idx < images_count; ++idx) {
        ...
        Mat mesh_cpu_x;
        Mat mesh_cpu_y;
        mesh_cpu_x = Mat(N, M, CV_32FC1);
        mesh_cpu_y = Mat(N, M, CV_32FC1);
        ...
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                mesh_cpu_x.at<float>(i, j) = x(2 * (j + i * M));
                mesh_cpu_y.at<float>(i, j) = x(2 * (j + i*M) + 1);
            }
        }
        ...
        /* InterpolationFlags f = INTER_LINEAR; */
        /* cuda::resize(gpu_small_mesh_x, big_x, mesh_size, f); */
        /* cuda::resize(gpu_small_mesh_y, big_y, mesh_size, f); */
        ...
        custom_resize(gpu_small_mesh_x, x_mesh[idx], mesh_size);
        custom_resize(gpu_small_mesh_y, y_mesh[idx], mesh_size);
    }
}
...
void calibrateMeshWarp(vector<Mat> &full_imgs, vector<ImageFeatures> &features,
                       ...
                       double work_scale) {
    if (enable_local_new) {
        calibrateMeshWarp2(full_imgs, features,
                           pairwise_matches, x_mesh,
                           y_mesh, x_maps,
                           y_maps, focal_length, compose_scale,
                           work_scale);
        return;
    }
    vector<Size> mesh_size(full_imgs.size());
